...
import top.fifthlight.bazel.worker.api.WorkResponse;
...
public class MappingMerger extends Worker {
    ...
    protected int handleRequest(WorkRequest request, PrintWriter out) {
        try {
            var args = request.arguments().toArray(new String[0]);
            return processArgs(args, out);
        } catch (Exception ex) {
    ...
    private int processArgs(String[] args, PrintWriter out) {
        try {
